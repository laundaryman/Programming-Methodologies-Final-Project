...
#define HEIGHT 792		// window's height  // WIDTH=HIGHT=unit*(2n+60)   (WIDTH,unit)    기본(600,10) 틀 (720,10)  조금 키움 (660,11) (792,11)    확장 (720,12) (864,12) 커질수록 글자 대칭이 깨짐
#define UNIT 11
...
int display_stat = 0; // display 상태. 0이면 맨처음상태. 1이면 게임중. 2이면 esc눌렀을 때. 3이면 다시시작 .5이면 죽는 순간. 6이면 좀비를 먹는 순간,7이면 fever 8이 2인 모드
...
int screen_mode = 0; // 0 빨강 1하양

int dead_player = 0; //죽은애가 누군지 0 0번 1 1번 2 둘다
...
void shake() {  // 이걸 실행시키면 흔들기 시작합니다
	shake_time =20;
	//shake_time = 150;
}
void redscreen(int mode) { // 빨간화면 시작
	redscreen_time = 22;
	screen_mode = mode;
}

void shake_handle() { // 흔들기 함수
	float size = 2.0;
...
void draw_redscreen() {
	if (redscreen_time > 0) {
		if(screen_mode==0)glColor4f(1, 0, 0, 0.1+(redscreen_time/22.0)*0.5); //빨강
		else if (screen_mode == 1)glColor4f(1, 1, 1, 0.1 + (redscreen_time / 22.0)*0.5); // 하양
		else if (screen_mode == 2)glColor4f(1, 0, 0,  0.1+ (redscreen_time / 22.0)*0.3); // 약한빨강
		else if (screen_mode == 3)glColor4f(1, 1, 1, 0.1 + (redscreen_time / 22.0)*0.4); // 약한하양
...
void processNormalKeys(unsigned char key, int x, int y) {
	...
	else if (key == 'R') bd.record_reset();// r누르면 기록 리셋 (숨겨진 기능)
	...
	if (display_stat == 0) {
		if (info_mode == 0) {
			if (no_key == 4 && vsmode == 1) {
				...
				for (int i = 0; z <= Zombienum; i++) {  // 좀비 생성
					int zx = rand() & 60;
					int zy = rand() % 50;
...
void idle() {
	/* Implement: Move the square */
		...
		if (end1 - start > 1000 / fps) {
			...
			}//restart display가 켜짐. 게임중이었다가 이긴 상황일때만!
			if (display_stat == 1) {
				...
				if (play.life <= 0) {
					bd.game_stat = 4;
					display_stat = 3;
				}// 라이프가 0이면 게임스텟을 4로 만듬 (게임오버),displaystat도 3으로됨

				

				if (play.player_stat == 2 && chk == 0) {  // 플레이어가 땅밟으면 칠하기 && 또 라이프를 잃은 경우가 아니어야함 (동시에 칠하면서 라이프를잃는경우를 제외시키기 위해)
				...
				}   // 좀비가 전부 죽으면 게임스텟을 2로 만듬(이긴 상태), displaystat도 3으로됨
				

				if ((int)bd.fever == 99&&bd.game_stat!=2) {
					bd.set_stat(3);
					bd.isfevermode = 1;
				}  // fever모드의 경우 게임스텟을 3으로 만듬.
				...
				int chk2 = 0;
				for (int i = 0; i < Zombienum; i++) {  // 좀비가 땅에 먹히면 좀비를 죽임
			...
			if (display_stat == 8) {
				...
				int chk1 = 0;  // 목숨잃었는지 체크하는 변수
					...
					if (bd.game_stat == 0) {
						...
						if (bd.game_time >= bd.time_limit) { // 시간초과
						...
						if (bd.ground_percent >= bd.wincondition_2) // 많이 먹음
						{
							bd.game_stat = 5;
							display_stat = 3;
							return;
						}
						if (bd.ground_percent1 >= bd.wincondition_2) // 많이먹음
						{
							bd.game_stat = 6;
							display_stat = 3;
							return;
				...
				if (play1.player_stat == 2 && chk == 0) {  // 플레이어가 땅밟으면 칠하기 && 또 라이프를 잃은 경우가 아니어야함 (동시에 칠하면서 라이프를잃는경우를 제외시키기 위해)
				...
				if (pause_time > 0) { pause_time--; }
					...
					if (vsmode == 1) {
						...
						if (play.life > 0) {
							...
							play.loose_life(bd.board, bd.boundary,bd.ground_board);  // 이거 위에 pause타임을 설정하고 바로 포스트리디스플래이 시키는 방법도 해보자
							bd.set_stat(1);
							shake();
							redscreen(0);
						...
						if (dead_player ==2) {
							...
							if(play.life>0)play.loose_life(bd.board, bd.boundary, bd.ground_board);  // 이거 위에 pause타임을 설정하고 바로 포스트리디스플래이 시키는 방법도 해보자
							...
							play1.loose_life(bd.board, bd.boundary, bd.ground_board);  // 이거 위에 pause타임을 설정하고 바로 포스트리디스플래이 시키는 방법도 해보자
							//bd.set_stat(1);
							shake();
							redscreen(0);
...
void renderScene() {
	...
	if (display_stat == 0) {  // 맨처음 난이도 설정
		...
		if (bd.isfevermode==1 && clock() % 500 <= 1000 / fps) {  // 얼마안남으면빨갛게
			redscreen(3);
		}
	...
	else if (display_stat == 2) {  // esc눌렀을때 화면
		glColor3f(1, 1, 1);
	...
	else if (display_stat == 3) {  // 게임 끝났을떄 화면
		bd.draw();
		bd.new_efferc_draw();
		...
		if (bd.game_time >= bd.time_limit-10000&&(int)bd.game_time%1000<=1000/fps) {  // 얼마안남으면빨갛게
			redscreen(2);
		}
		draw_redscreen();
